...
class edit_area_compressor
{
    public function compressor($param)
    {
    ...
    public function check_gzip_use()
    {
        $encodings = [];
    ...
    public function load_files()
    {
        ...
        $loader = preg_replace("/(t\.sub_scripts_to_load=\s*)\[([^\]]*)\];/e", "\$this->replace_scripts('sub_script_list', '\\1', '\\2')", $loader);
        ...
        $this->datas = preg_replace(["/(\b)EditAreaLoader(\b)/", "/(\b)editAreaLoader(\b)/", "/(\b)editAreas(\b)/"], ['EAL', 'eAL', 'eAs'], $this->datas);
        ...
        $sub_scripts = preg_replace(["/(\b)editAreaLoader(\b)/", "/(\b)editAreas(\b)/", "/(\b)editArea(\b)/", "/(\b)EditArea(\b)/"], ['eAL', 'eAs', 'eA', 'EA'], $sub_scripts);
        ...
        foreach ($last_comp as $key => $val) {
            $js_replace .= '.replace(/' . $key . "/g,'" . str_replace(["\n", "\r"], ['\n', '\r'], $val) . "')";
        }
    ...
    public function get_content($end_uri)
    {
    ...
    public function get_javascript_content($end_uri)
    {
    ...
    public function compress_javascript(&$code)
    {
        if ($this->param['compress']) {
            // remove all comments
            //	(\"(?:[^\"\\]*(?:\\\\)*(?:\\\"?)?)*(?:\"|$))|(\'(?:[^\'\\]*(?:\\\\)*(?:\\'?)?)*(?:\'|$))|(?:\/\/(?:.|\r|\t)*?(\n|$))|(?:\/\*(?:.|\n|\r|\t)*?(?:\*\/|$))
    ...
    public function get_css_content($end_uri)
    {
    ...
    public function get_html_content($end_uri)
    {
        $code = $this->get_content($end_uri);
        //$code= preg_replace('/(\"(?:\\\"|[^\"])*(?:\"|$))|' . "(\'(?:\\\'|[^\'])*(?:\'|$))|(?:\/\/(?:.|\r|\t)*?(\n|$))|(?:\/\*(?:.|\n|\r|\t)*?(?:\*\/|$))/s", "$1$2$3", $code);
        $code = preg_replace('/(( |\t|\r)*\n( |\t)*)+/s', ' ', $code);
    ...
    public function prepare_string_for_quotes(&$str)
    {
    ...
    public function replace_scripts($var, $param1, $param2)
    {
    ...
    public function file_put_contents($file, &$content, $mtime = -1)
    {
        if ($mtime == -1) {
            $mtime = time();
        }
