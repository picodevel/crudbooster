...
<head>
    ...
    <!-- Theme style -->
    <link href="{{ asset("vendor/crudbooster/assets/adminlte/dist/css/AdminLTE.min.css")}}?v=1.1" rel="stylesheet" type="text/css"/>
            ...
            @if($module)
                <h1>
                    ...
                    @if(CRUDBooster::getCurrentMethod() == 'getIndex')
                        @if($button_show)
                            <a href="{{ CRUDBooster::mainpath() }}" id='btn_show_data' class="btn btn-sm btn-primary"
                ...
                <!--ADD ACTIon-->
                    @if(!empty($index_button))
                        ...
                        @foreach($index_button as $ib)
                            <a href='{{$ib["url"]}}' id='{{\Illuminate\Support\Str::slug(isset($ib["id"]) ? $ib["id"] : $ib["label"])}}' class='btn {{($ib['color'])?'btn-'.$ib['color']:'btn-primary'}} btn-sm'
