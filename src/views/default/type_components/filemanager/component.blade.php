<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style='{{@$form["style"]}}'>
    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        ...
        @if($value)
            ...
            @else
                <p><a data-lightbox="roadtrip" href="{{ ($value)?asset($value):'' }}">
                    <img id='holder-{{$name}} {{ ($value)?'src='.asset($value):'' }} style="margin-top:15px;max-height:100px;"></a>
                </p>
            @endif
    ...
    @push('bottom')
        <script type="text/javascript">
            document.addEventListener("DOMContentLoaded", function(){
                $('#lfm-<?php echo $name ?>').filemanager('file', {prefix: "<?php echo url(config('lfm.prefix')) ?>"});
            });
        </script>
...
@else
    ...
    <script type="text/javascript">
        document.addEventListener("DOMContentLoaded", function(){
            $('#lfm-<?php echo $name ?>').filemanager('images', {prefix: "<?php echo url(config('lfm.prefix')) ?>"});
        });
