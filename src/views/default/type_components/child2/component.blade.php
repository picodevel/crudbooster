...
foreach($data_child as $d):
    ...
    ?>
                ...
                if ($col['type'] == 'select') {
                    ...
                    $hiddens = $hiddens . "<input type='hidden' name='" . $name . '-' . $col['name'] . "' value='" . $d->{$col['name']} . "'>";

                } else {
                    $required = $col['required'] == 1 ? 'required' : '';
                    $readonly = $col['readonly'] == true ? 'readonly' : '';
                    $cls = $name . $col['name'];
                    $nm = 'name=' . $name . '-' . $col['name'] . '[]';
                    $id = 'id=' . $name . $col['name'] . '-' . $idx;
                    $row = "data-row='" . $idx . "'";
                    $val = 'value=' . $d->{$col['name']};
                    if($col['type'] == 'number') {
                        echo "<input class='form-control " . $cls . "' autocomplete='off' type='number' " . $row . ' ' . $nm . ' ' . $id . ' ' . $readonly . ' ' . $val . ' ' . $required . '>';
                    } elseif($col['type'] == 'money') {
                        echo "<input class='form-control inputMoney " . $cls . "' autocomplete='off' type='text' " . $row . ' ' . $nm . ' ' . $readonly . ' ' . $id . ' ' . $val . ' ' . $val . ' ' . $required . '>';
                    } else {
                        echo "<input class='form-control " . $cls . "' autocomplete='off' type='text' " . $row . ' ' . $nm . ' ' . $id . ' ' . $readonly . ' ' . $required . '>';
                    }
    ...
    ?>
                                    ...
                                    <script>
                                        $(function () {
                                            @foreach($form['columns'] as $col)
                                                @if($col['type'] == 'money')
                                                $('.inputMoney.{{$name.$col['name']}}').priceFormat({!! json_encode(array_merge(array(
                                                                'prefix'                => '',
                    ...
                    <button class="btn btn-info" data-toggle="modal" data-target="#modal-child2-item" type="button"><i class="fa fa-plus" aria-hidden="true"></i> Add</button>
                </div>
                <!-- /.box-body -->
            </div>
...
foreach ($sums as $value) {
    ...
    $sum_names[] = $fields[0];
    $scripts_inside[] = "
                    $('body').on('change', '.$name$fields[0]',function(){
                        formulaSum" . $name . str_slug($fields[0], '') . '();
                    });
                    ';
    $scripts_outside[] = '
                    function formulaSum' . $name . str_slug($fields[0], '') . "(){
                        var sum = 0;
                        $('.$name$fields[0]').each(function(i, obj) {
                            sum = sum + ($(this).val() != '' ? parseInt($(this).val().replace(/,/g,'')) : 0);    
                        });
                        $('#$fields[1]').val(sum);
                        $('#$fields[1]').trigger('change');
                        $('#$fields[1]').trigger('blur');
                    }
                    ";
}
?>
        ...
        @push('bottom')
            <script type="text/javascript">
                @foreach($scripts_outside as $value)
                    {!! $value !!}
                @endforeach
                $(function () {
                    @foreach($scripts_inside as $value)
                        {!! $value !!}
                    @endforeach
                })
            </script>
...
<div id="modal-child2-item" class="modal fade" role="dialog">
...
$formula_function_row_name = 'formulaRow' . str_slug($name . $col['name'], '');
...
foreach ($form['columns'] as $c) {
    if (strpos($formula, '[' . $c['name'] . ']') !== false) {
        $script_onchange .= "
                                ...
                                ";
        $script_row_onchange .= "
                                $('body').on('change', '.$name$c[name]',function(){
                                    $formula_function_row_name($(this).data('row'));
                                });
                                ";
    }
    $formula = str_replace('[' . $c['name'] . ']', "\$('#" . $name . $c['name'] . "').val().replace(/,/g,'')", $formula);
    $formula_row = str_replace('[' . $c['name'] . ']', "\$('#" . $name . $c['name'] . "'+'-'+id).val().replace(/,/g,'')", $formula_row);
}
?>
                    @push('bottom')
                        <script type="text/javascript">
                            function {{ $formula_function_name }}() {
                                var v = {!! $formula !!};
                                $('#{{$name_column}}').val(v).trigger('blur');
                            }

                            function {{ $formula_function_row_name }}(id) {
                                var v = {!! $formula_row !!};
                                $('#{{$name_column}}-'+id).val(v);
                                $('#{{$name_column}}-'+id).trigger('change');
                                $('#{{$name_column}}-'+id).trigger('blur');
                            }
            ...
            @push('bottom')
                <script type="text/javascript">
                    ...
                    function resetForm{{$name}}() {
                        $('#panel-form-{{$name}} .child-form-area').find("input[type=text],input[type=number],select,textarea").val('');
                        $('#panel-form-{{$name}} .child-form-area').find(".select2").val('').trigger('change');
                    }
                    ...
                    function deleteRow{{$name}}(t) {
                        ...
                        if (confirm("{{trans('crudbooster.delete_title_confirm')}}")) {
                            ...
                            @foreach($sum_names as $value)
                                formulaSum{{$name.$value}}();
                            @endforeach
                        }
                    }
                    ...
                    function addToTable{{$name}}() {
                        ...
                        @foreach($form['columns'] as $c)
                            ...
                            @elseif($c['type']=='hidden')
                                hiddens = hiddens + "<input type='hidden' name='"+"{{$name}}-{{$c['name']}}"+"[]' value='"+$("#{{$name.$c['name']}}").val()+"'>";
                            @else

                                var required = "{{$c['required'] == true ? 'required' : ''}}";
                                var readonly = "{{$c['readonly'] == true ? 'readonly' : ''}}";
                                var input = "";
                                var idx =  $('#table-{{$name}} tbody tr').length;
                                var cls = "{{$name}}{{$c['name']}}";
                                var name = "name='"+"{{$name}}-{{$c['name']}}"+"[]'";
                                var id = "id='{{$name}}{{$c['name']}}-"+idx+"'";
                                var row = "data-row='"+idx+"'";
                                var value = "value='"+$("#{{$name.$c['name']}}").val()+"'";
                                @if($c['type'] == 'number')
                                    input = "<input class='form-control "+cls+"' autocomplete='off' type='number' "+row+" "+name+" "+id+" "+value+" "+readonly+" "+required+">";
                                @elseif($c['type'] == 'money')
                                    input = "<input class='form-control inputMoney "+cls+"' autocomplete='off' type='text' "+row+" "+name+" "+id+" "+value+" "+readonly+" "+required+">";
                                @else
                                    input = "<input class='form-control "+cls+"' autocomplete='off' type='text' "+row+" "+name+" "+id+" "+value+" "+readonly+" "+required+">";
                                @endif

                                trRow += "<td class='{{$c['name']}}'>"+input+"</td>";
                            @endif
                        @endforeach
                            trRow += "<td>" +hiddens+
                        ...
                        $('#table-{{$name}} tbody input').each(function () {
                            var el = document.getElementById($(this).attr("id"));
                            if(el !== null && el !== 'undefined' && JSON.stringify(el) != 'null'){
                                el.value = $(this).val();
                                $(this).trigger('change');
                                if($(this).hasClass('inputMoney')){
                                    $(this).priceFormat({!! json_encode(array_merge(array(
                                                    'prefix'                => '',
                                                    'thousandsSeparator'    => $form['dec_point'] ? : ',',
                                                    'centsLimit'            => $form['decimals'] ? : '0',
                                                    'clearOnEmpty'          => false,
      ...
      <div class="modal-footer">
        <input type='button' class='btn btn-danger' id="btn-reset-form-{{$name}}" onclick="resetForm{{$name}}()"
                           value='{{trans("crudbooster.button_reset")}}'/>
        <input type='button' id='btn-add-table-{{$name}}' class='btn btn-success' onclick="addToTable{{$name}}()"
