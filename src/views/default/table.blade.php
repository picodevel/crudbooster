        ...
        <script>
            $(function () {
                ...
                $('.btn-export-data').click(function () {
                    var selected = "";
                    $("input[name='checkbox[]']").each(function(i,input){
                        if($(input).is(":checked")){
                            if(selected != ""){
                                selected += ",";
                            }
                    ...
                    $("input[name='selected_data']").val(selected);
                ...
                $(".filter-combo").change(function () {
                    ...
                    switch (n) {
                        default:
                            //.val('')
                            filter_value.removeAttr('placeholder').prop('disabled', true);
                            p.find('.between-group').find('input').prop('disabled', true);
                            break;
                        case 'like':
                        case 'not like':
                ...
                <div class="modal-content">
                    ...
                    <form method='get' action=''>
                        <div class="modal-body">
                            ...
                            <?php if (isset($col['image']) || isset($col['download']) || $col['filterable'] === false) {
                                continue;
                            }?>
                            ...
                            <div class='form-group'>
                                ...
                                <div class='row-filter-combo row'>
                                    ...
                                    <div class='col-sm-3'>
                                        <select name='filter_column[{{$col["field_with"]}}][type]' data-type='{{$col["type_data"]}}'
                                            ...
                                            @if(in_array($col['type_data'],['string','varchar','text','char']))
                                                <option {{ (CRUDBooster::getTypeFilter($col["field_with"]) == 'like' || @$col["field_compare"] == 'like')?"selected":"" }} value='like'>{{trans("crudbooster.filter_like")}}</option> @endif
                                            @if(in_array($col['type_data'],['string','varchar','text','char']))
                                                <option {{ (CRUDBooster::getTypeFilter($col["field_with"]) == 'not like' || @$col["field_compare"] == 'not like')?"selected":"" }} value='not like'>{{trans("crudbooster.filter_not_like")}}</option>@endif

                                            <option typeallow='all'
                                                    {{ (CRUDBooster::getTypeFilter($col["field_with"]) == '=' || @$col["field_compare"] == '=')?"selected":"" }} value='='>{{trans("crudbooster.filter_equal_to")}}</option>
                                            @if(in_array($col['type_data'],['int','integer','double','float','decimal','time']))
                                                <option {{ (CRUDBooster::getTypeFilter($col["field_with"]) == '>=' || @$col["field_compare"] == '>=')?"selected":"" }} value='>='>{{trans("crudbooster.filter_greater_than_or_equal")}}</option>@endif
                                            @if(in_array($col['type_data'],['int','integer','double','float','decimal','time']))
                                                <option {{ (CRUDBooster::getTypeFilter($col["field_with"]) == '<=' || @$col["field_compare"] == '<=')?"selected":"" }} value='<='>{{trans("crudbooster.filter_less_than_or_equal")}}</option>@endif
                                            @if(in_array($col['type_data'],['int','integer','double','float','decimal','time']))
                                                <option {{ (CRUDBooster::getTypeFilter($col["field_with"]) == '<' || @$col["field_compare"] == '>')?"selected":"" }} value='<'>{{trans("crudbooster.filter_less_than")}}</option>@endif
                                            @if(in_array($col['type_data'],['int','integer','double','float','decimal','time']))
                                                <option {{ (CRUDBooster::getTypeFilter($col["field_with"]) == '>' || @$col["field_compare"] == '>')?"selected":"" }} value='>'>{{trans("crudbooster.filter_greater_than")}}</option>@endif
                                            <option typeallow='all'
                                                    {{ (CRUDBooster::getTypeFilter($col["field_with"]) == '!=')?"selected":"" }} value='!=' || @$col["field_compare"] == '!='>{{trans("crudbooster.filter_not_equal_to")}}</option>
                                            <option typeallow='all'
                                                    {{ (CRUDBooster::getTypeFilter($col["field_with"]) == 'in' || @$col["field_compare"] == 'in')?"selected":"" }} value='in'>{{trans("crudbooster.filter_in")}}</option>
                                            <option typeallow='all'
                                                    {{ (CRUDBooster::getTypeFilter($col["field_with"]) == 'not in' || @$col["field_compare"] == 'not in')?"selected":"" }} value='not in'>{{trans("crudbooster.filter_not_in")}}</option>
                                            @if(in_array($col['type_data'],['date','time','datetime','int','integer','double','float','decimal','timestamp', 'varchar']))
                                                <option {{ (CRUDBooster::getTypeFilter($col["field_with"]) == 'between' || @$col["field_compare"] == 'between')?"selected":"" }} value='between'>{{trans("crudbooster.filter_between")}}</option>@endif
                                    ...
                                    <div class='col-sm-4'>
                                        @if(isset($col['field_options']))
                                            <?php
                                                $options = $col['field_options'];
                                ?>
                                                <select class='filter-value form-control' style="{{ (CRUDBooster::getTypeFilter($col["field_with"]) == 'between')?"display:none":"display:block"}}"
                                                {{@$col["field_compare"] != "" ? "" : "disabled"}} name='filter_column[{{$col["field_with"]}}][value]'>
                                                    <option value="">
                                                    </option>
                                                    @foreach($options as $key => $value)
                                                    <option {{ ((!is_array(CRUDBooster::getValueFilter($col["field_with"])))?CRUDBooster::getValueFilter($col["field_with"]):"") == $key ? "selected" : "" }} value="{{$key}}">
                                                        {{$value}}
                                                    </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class='col-sm-6' style="padding-right: 0px">
                                                <div class='input-group {{ ($col["type_data"] == "time")?"bootstrap-timepicker":"" }}'>
                                                    ...
                                                    <input
                                                            ...
                                                            type='text'
                                                            class='filter-value-between form-control {{ (in_array($col["type_data"],["date","datetime","timestamp"]))?"datepicker":"" }}'
                                                            {{ (in_array($col["type_data"],["date","datetime","timestamp"]))?"readonly":"" }} placeholder='{{$col["label"]}} {{trans("crudbooster.filter_from")}}'
                                            ...
                                            <div class='col-sm-6' style="padding-left: 0px">
                                                <div class='input-group {{ ($col["type_data"] == "time")?"bootstrap-timepicker":"" }}'>
                                                    ...
                                                    <input
                                                            ...
                                                            type='text'
                                                            class='filter-value-between form-control {{ (in_array($col["type_data"],["date","datetime","timestamp"]))?"datepicker":"" }}'
                                                            {{ (in_array($col["type_data"],["date","datetime","timestamp"]))?"readonly":"" }} placeholder='{{$col["label"]}} {{trans("crudbooster.filter_to")}}'
                                                            name='filter_column[{{$col["field_with"]}}][value][]' value='<?php
                                $value = CRUDBooster::getValueFilter($col['field_with']);
                        ...
                        <div class="modal-body">
                            ...
                            <div class='form-group'>
                                ...
                                @foreach($columns_export as $col)
                                    <div class='checkbox inline' style="padding-right: 10px;">
                                        <label>
                                            <input type='checkbox' {!! isset($col['selected']) && !$col['selected'] ? '' : 'checked' !!} name='columns[]' value='{{$col["label"]}}'>{{$col["label"]}}
                                        </label>
                                    </div>
                                @endforeach
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name='fileformat' class='form-control'>
                                    @if(count($export_format) == 0 || in_array('pdf',$export_format))
                                    <option value='pdf'>PDF</option>
                                    @endif
                                    @if(count($export_format) == 0 || in_array('xls',$export_format))
                                    <option value='xls'>Microsoft Excel (xls)</option>
                                    @endif
                                    @if(count($export_format) == 0 || in_array('csv',$export_format))
